    ...
    <loading-view :loading="initialLoading" :dusk="resourceName + '-index-component'">
        ...
        <div class="flex">
            ...
            <div class="w-full flex items-center mb-6">
                ...
                <button @click="sync" class="btn btn-default btn-primary inline-flex items-center relative"
                        type="button">
                    <span :class="{ invisible: syncing }"> {{__('Sync')}} </span>
    ...
    export default {
        ...
        methods: {
            sync(event) {
                ...
                this.syncing = true;
                return Nova.request()
                    .get(`/nova-vendor/nova-theme/sync`)
                    .then(response => {
                        this.$toasted.show(this.__('Themes Synced Successfully'), {type: 'success'})
                        this.getResources();
                    }).finally(() => {
                        this.syncing = false;
                    });
